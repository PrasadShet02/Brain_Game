...
butset=set([b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14,b15,b16])

def colorswitch(x):
        x.config(bg='grey')
        x.after(2000,lambda: x.config(bg='white'))
        
y=set(random.sample(butset,6))
for i in y:
    colorswitch(i)
...
def checking(s):
    if s in y:
        ...
        l2.grid(row=5,column=2)
        mb.showinfo("Visual memory","Oops, thanks for playing! You cleared 2 levels!")
        root.destroy()
        exit()
